...
**Value over a vanilla agent**: A vanilla agent can retrieve job events and read error messages. But it cannot determine whether the error is a platform issue (host unreachable, capacity exhaustion) or a code issue (bad playbook logic, missing variable). This document gives the agent the classification framework and correlation patterns to make that determination -- backed by Red Hat's troubleshooting guide.
