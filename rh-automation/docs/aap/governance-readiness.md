...
**Value over a vanilla agent**: A vanilla agent with MCP access can list users, credentials, and templates. But it cannot interpret whether that configuration is *good* or *dangerous*. This document gives the agent the knowledge to make that judgment -- and every judgment is traceable to an official Red Hat source.
